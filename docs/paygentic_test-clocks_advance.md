...
```
paygentic test-clocks advance [flags]
```

### Examples

```
  paygentic test-clocks advance --id <id>
```
...
```
  ...
  -b, --body-param string   JSON value (one of: { currentTime: date-time } | { advanceBy: string })
  -h, --help                help for advance
  -i, --id string           Test clock ID [required]
```
