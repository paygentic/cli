#
# paygentic CLI Installation Script for Windows
# This script downloads and installs the latest version of the paygentic CLI
#
# Usage:
#   iwr -useb https://raw.githubusercontent.com/paygentic/cli/main/scripts/install.ps1 | iex
#   or
#   Invoke-WebRequest -Uri https://raw.githubusercontent.com/paygentic/cli/main/scripts/install.ps1 -UseBasicParsing | Invoke-Expression
#
# Options:
#   $env:PAYGENTIC_INSTALL_DIR - Installation directory (default: $env:LOCALAPPDATA\Programs\paygentic)
#   $env:PAYGENTIC_VERSION     - Specific version to install (default: latest)
#
...
# Configuration
$Repo = "paygentic/cli"
$BinaryName = "paygentic.exe"
$DefaultInstallDir = Join-Path $env:LOCALAPPDATA "Programs\paygentic"
$InstallDir = if ($env:PAYGENTIC_INSTALL_DIR) { $env:PAYGENTIC_INSTALL_DIR } else { $DefaultInstallDir }
$Version = if ($env:PAYGENTIC_VERSION) { $env:PAYGENTIC_VERSION } else { "latest" }
...
function Install-CLI {
    Write-ColorOutput "Installing paygentic CLI..." -Color Green
    ...
    $archiveName = "paygentic_Windows_$arch.zip"
    $downloadUrl = "https://github.com/$Repo/releases/download/$Version/$archiveName"

    Write-ColorOutput "Downloading from: $downloadUrl" -Color Cyan

    # Create temporary directory
    $tempDir = Join-Path $env:TEMP "paygentic-install-$(New-Guid)"
    ...
    try {
        ...
        Write-ColorOutput "paygentic $Version has been installed to $binaryPath" -Color Green
        ...
        Write-ColorOutput "Installation successful! Run 'paygentic --help' to get started." -Color Green
