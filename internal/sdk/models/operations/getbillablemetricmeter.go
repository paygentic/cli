// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 6c5e53bf2b0d

package operations

import (
	"encoding/json"
	"fmt"
	"github.com/paygentic/cli/internal/sdk/models/components"
	"github.com/paygentic/cli/internal/sdk/sdkinternal/utils"
	"time"
)

// GetBillableMetricMeterWindowSize - Time bucket granularity
type GetBillableMetricMeterWindowSize string

const (
	GetBillableMetricMeterWindowSizeMinute GetBillableMetricMeterWindowSize = "MINUTE"
	GetBillableMetricMeterWindowSizeHour   GetBillableMetricMeterWindowSize = "HOUR"
	GetBillableMetricMeterWindowSizeDay    GetBillableMetricMeterWindowSize = "DAY"
	GetBillableMetricMeterWindowSizeMonth  GetBillableMetricMeterWindowSize = "MONTH"
)

func (e GetBillableMetricMeterWindowSize) ToPointer() *GetBillableMetricMeterWindowSize {
	return &e
}
func (e *GetBillableMetricMeterWindowSize) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "MINUTE":
		fallthrough
	case "HOUR":
		fallthrough
	case "DAY":
		fallthrough
	case "MONTH":
		*e = GetBillableMetricMeterWindowSize(v)
		return nil
	default:
		return fmt.Errorf("invalid value for GetBillableMetricMeterWindowSize: %v", v)
	}
}

type GetBillableMetricMeterRequest struct {
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Start of query window (ISO 8601)
	From time.Time `queryParam:"style=form,explode=true,name=from"`
	// End of query window (ISO 8601)
	To time.Time `queryParam:"style=form,explode=true,name=to"`
	// Filter by customer/user ID
	Subject *string `queryParam:"style=form,explode=true,name=subject"`
	// Time bucket granularity
	WindowSize *GetBillableMetricMeterWindowSize `queryParam:"style=form,explode=true,name=windowSize"`
	// JSON-encoded dimension filter (e.g. {"key":"value"})
	FilterGroupBy *string `queryParam:"style=form,explode=true,name=filterGroupBy"`
	// Comma-separated dimension keys
	GroupBy *string `queryParam:"style=form,explode=true,name=groupBy"`
}

func (g GetBillableMetricMeterRequest) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(g, "", false)
}

func (g *GetBillableMetricMeterRequest) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &g, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (g *GetBillableMetricMeterRequest) GetID() string {
	if g == nil {
		return ""
	}
	return g.ID
}

func (g *GetBillableMetricMeterRequest) GetFrom() time.Time {
	if g == nil {
		return time.Time{}
	}
	return g.From
}

func (g *GetBillableMetricMeterRequest) GetTo() time.Time {
	if g == nil {
		return time.Time{}
	}
	return g.To
}

func (g *GetBillableMetricMeterRequest) GetSubject() *string {
	if g == nil {
		return nil
	}
	return g.Subject
}

func (g *GetBillableMetricMeterRequest) GetWindowSize() *GetBillableMetricMeterWindowSize {
	if g == nil {
		return nil
	}
	return g.WindowSize
}

func (g *GetBillableMetricMeterRequest) GetFilterGroupBy() *string {
	if g == nil {
		return nil
	}
	return g.FilterGroupBy
}

func (g *GetBillableMetricMeterRequest) GetGroupBy() *string {
	if g == nil {
		return nil
	}
	return g.GroupBy
}

type GetBillableMetricMeterResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// Usage data
	UsageResponse *components.UsageResponse
}

func (g GetBillableMetricMeterResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(g, "", false)
}

func (g *GetBillableMetricMeterResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &g, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (g *GetBillableMetricMeterResponse) GetHTTPMeta() components.HTTPMetadata {
	if g == nil {
		return components.HTTPMetadata{}
	}
	return g.HTTPMeta
}

func (g *GetBillableMetricMeterResponse) GetUsageResponse() *components.UsageResponse {
	if g == nil {
		return nil
	}
	return g.UsageResponse
}
