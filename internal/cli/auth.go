...
func initAuthCmd(parent *cobra.Command) error {
	authCmd := &cobra.Command{
		...
		Long: `Manage authentication credentials for paygentic.
...
Subcommands:
  ...
  logout  - Clear all stored credentials`,
	...
	authCmd.AddCommand(&cobra.Command{
		...
		Long: `Interactively configure authentication credentials for paygentic.
...
func runAuthLoginCmd(cmd *cobra.Command, args []string) error {
	...
	if output.IsAgentMode() {
		return output.AgentModeError(cmd,
			...
			[]string{
				fmt.Sprintf("Set credentials via environment variables (prefix: %s_)", "PAYGENTIC"),
				"Pass credentials directly as CLI flags for each command",
				fmt.Sprintf("Run '%s auth whoami' to verify current authentication", "paygentic"),
			},
		)
	}
