...
import (
	...
	"github.com/paygentic/cli/internal/cli/disputes"
	"github.com/paygentic/cli/internal/cli/entitlements"
	"github.com/paygentic/cli/internal/cli/entitlementsv0"
	...
	"github.com/paygentic/cli/internal/cli/usageevents"
...
func NewRootCommand() (*cobra.Command, error) {
	...
	rootCmd := &cobra.Command{
		Use:           "paygentic",
		Short:         "Paygentic API: The Paygentic API provides billing infrastructure for usage-based and subscription monetization — customers, subscriptions, usage metering, invoicing, entitlements, and payments",
		Long:          "Paygentic API: The Paygentic API provides billing infrastructure for usage-based and subscription monetization — customers, subscriptions, usage metering, invoicing, entitlements, and payments.\n\nSee the [Quickstart](https://docs.paygentic.io/getting-started/quickstart) to go from zero to billing in a handful of steps.",
		SilenceUsage:  true,
		SilenceErrors: true,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := config.Init("paygentic", "PAYGENTIC"); err != nil {
				return err
			}
	...
	if err := billablemetrics.InitBillableMetricsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init billable-metrics: %w", err)
	}
	if err := customers.InitCustomersRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init customers: %w", err)
	}
	if err := disputes.InitDisputesRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init disputes: %w", err)
	}
	if err := entitlementsv0.InitEntitlementsV0Root(rootCmd); err != nil {
		return nil, fmt.Errorf("init entitlements-v0: %w", err)
	}
	if err := entitlements.InitEntitlementsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init entitlements: %w", err)
	}
	if err := features.InitFeaturesRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init features: %w", err)
	}
	if err := fees.InitFeesRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init fees: %w", err)
	}
	if err := plans.InitPlansRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init plans: %w", err)
	}
	if err := prices.InitPricesRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init prices: %w", err)
	}
	if err := products.InitProductsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init products: %w", err)
	}
	if err := sources.InitSourcesRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init sources: %w", err)
	}
	if err := subscriptions.InitSubscriptionsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init subscriptions: %w", err)
	}
	if err := usageevents.InitUsageEventsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init usage-events: %w", err)
	}
	if err := users.InitUsersRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init users: %w", err)
	}
	if err := invoicesv2.InitInvoicesV2Root(rootCmd); err != nil {
		return nil, fmt.Errorf("init invoices-v2: %w", err)
	}
	...
	if err := costs.InitCostsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init costs: %w", err)
	}
	...
	rootCmd.PersistentFlags().String("bearer-auth", "", "API key authentication")
